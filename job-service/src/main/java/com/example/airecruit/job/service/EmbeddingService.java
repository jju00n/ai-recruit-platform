 ...
 * OPENAI_API_KEY 미설정 시 null 반환 → 임베딩 없이 저장 (kNN 검색 불가).
 */
@Service
@Slf4j
public class EmbeddingService {
    ...
    private static final int MAX_CHARS = 8000; // ≈ 2000 토큰
    ...
    public void init() {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            log.warn("[EmbeddingService] OPENAI_API_KEY 미설정 — 임베딩 없이 저장됩니다.");
        } else {
            log.info("[EmbeddingService] OpenAI 임베딩 활성화 (key={}...)", apiKey.substring(0, Math.min(8, apiKey.length())));
        }
    }
    ...
    public float[] embed(String text) {
        ...
        try {
            log.info("[EmbeddingService] 임베딩 요청 - 텍스트 {}자", input.length());
